...
const ReposPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'repositories' | 'issues'>('repositories');
  ...
  const filteredIssues = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(issue => 
        ...
        issue.language.some(lang => lang.toLowerCase().includes(lowerSearchTerm))
      );
    }
    ...
    switch (issueSort) {
      case 'bounty-high':
        filtered.sort((a, b) => {
          const bountyA = a.multiplierActive && a.multiplierValue ? a.bounty * a.multiplierValue : a.bounty;
          const bountyB = b.multiplierActive && b.multiplierValue ? b.bounty * b.multiplierValue : b.bounty;
  ...
  const sortBadgeText = useMemo(() => {
    switch (issueSort) {
      case 'bounty-high': return 'Bounty: High to Low';
      case 'bounty-low': return 'Bounty: Low to High';
  ...
  const hasActiveFilters = issueFilter !== 'all' || searchTerm || issueSort !== 'newest';
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button 
          variant="outline" 
          size="sm"
          className="bg-gray-800 border-gray-600 hover:bg-gray-700 hover:border-gray-500 text-gray-200"
        >
      ...
      <DropdownMenuContent className="bg-gray-800 border-gray-600 text-gray-200">
        <DropdownMenuLabel className="text-gray-300">Filter Issues</DropdownMenuLabel>
        ...
        >
          ...
          {issueFilter === 'all' && <CheckSquare className="ml-2 h-4 w-4 text-blue-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueFilter === 'claimed' && <CheckSquare className="ml-2 h-4 w-4 text-blue-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueFilter === 'unclaimed' && <CheckSquare className="ml-2 h-4 w-4 text-blue-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueFilter === 'completed' && <CheckSquare className="ml-2 h-4 w-4 text-blue-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueFilter === 'active' && <CheckSquare className="ml-2 h-4 w-4 text-blue-400" />}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
  ...
  const SortMenu = () => (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="bg-gray-800 border-gray-600 text-gray-200">
        <DropdownMenuLabel className="text-gray-300">Sort Issues</DropdownMenuLabel>
        <DropdownMenuSeparator className="bg-gray-600" />
        <DropdownMenuItem 
        ...
        >
          ...
          {issueSort === 'newest' && <CheckSquare className="ml-2 h-4 w-4 text-purple-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueSort === 'oldest' && <CheckSquare className="ml-2 h-4 w-4 text-purple-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueSort === 'bounty-high' && <CheckSquare className="ml-2 h-4 w-4 text-purple-400" />}
        </DropdownMenuItem>
        <DropdownMenuItem 
        ...
        >
          ...
          {issueSort === 'bounty-low' && <CheckSquare className="ml-2 h-4 w-4 text-purple-400" />}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
  ...
  const desktopView = (
    <div className="flex flex-col gap-6 md:flex-row">
      ...
      <div className="w-full flex-shrink-0 rounded-lg bg-gray-900 p-4 shadow-lg md:w-1/2 lg:w-5/12">
        <h2 className="mb-3 border-b border-gray-700 pb-2 font-semibold text-2xl text-gray-200 flex items-center">
          <GitBranch className="mr-2 h-6 w-6 text-blue-400" />
          Repositories <span className="ml-2 text-blue-400">({repositories.length})</span>
        </h2>
        <div className="h-[70vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
      ...
      <div className="w-full rounded-lg bg-gray-900 p-4 shadow-lg md:w-1/2 lg:w-7/12">
        <div className="mb-3 flex items-center justify-between border-b border-gray-700 pb-2">
          <h2 className="font-semibold text-2xl text-gray-200 flex items-center">
          ...
          {selectedRepo && (
            <Button
              ...
              className="border border-gray-700 text-gray-300 hover:bg-gray-800 hover:text-red-400 transition-colors"
            >
          ...
          <div className="mb-4 flex flex-wrap gap-2 items-center">
            <div className="relative flex-grow max-w-md">
              ...
              <Input
                ...
                className="pl-8 bg-gray-800 border-gray-700 text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              />
              {searchTerm && (
                <button 
                ...
                >
                  <X className="h-4 w-4 text-gray-500 hover:text-red-400 transition-colors" />
                </button>
              )}
            </div>
            ...
            {hasActiveFilters && (
              <Button 
                ...
                className="text-gray-300 hover:text-red-400 border border-gray-700"
              >
        ...
        {selectedRepo && hasActiveFilters && (
          <div className="mb-4 flex flex-wrap gap-2">
            {issueFilter !== 'all' && (
              <Badge variant="outline" className="bg-gray-800 border-blue-600 text-blue-300">
                <Filter className="mr-1 h-3 w-3" /> {filterBadgeText}
              </Badge>
            )}
            {issueSort !== 'newest' && (
              <Badge variant="outline" className="bg-gray-800 border-purple-600 text-purple-300">
                <SortAsc className="mr-1 h-3 w-3" /> {sortBadgeText}
              </Badge>
            )}
            {searchTerm && (
              <Badge variant="outline" className="bg-gray-800 border-green-600 text-green-300">
        ...
        <div className="h-[calc(70vh-100px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
          {selectedRepo ? (
            ...
            <div className="flex flex-col h-40 items-center justify-center rounded-lg border-2 border-dashed border-gray-700 bg-gray-800/50">
  ...
  const mobileView = (
    <Tabs 
      value={activeTab} 
      onValueChange={(value) => setActiveTab(value as 'repositories' | 'issues')}
      className="md:hidden"
    >
      ...
      <TabsContent value="repositories" className="mt-4 bg-gray-900 rounded-lg p-4 shadow-lg">
        <h2 className="mb-3 border-b border-gray-700 pb-2 font-semibold text-2xl text-gray-200 flex items-center">
          <GitBranch className="mr-2 h-6 w-6 text-blue-400" />
          Repositories <span className="ml-2 text-blue-400">({repositories.length})</span>
        </h2>
      ...
      <TabsContent value="issues" className="mt-4 bg-gray-900 rounded-lg p-4 shadow-lg">
        <div className="mb-3 flex items-center justify-between border-b border-gray-700 pb-2">
          <h2 className="font-semibold text-2xl text-gray-200 flex items-center">
            <Code className="mr-2 h-6 w-6 text-purple-400" />
            Issues
        ...
        {selectedRepo && hasActiveFilters && (
          <div className="mb-4 flex flex-wrap gap-2">
            ...
            {searchTerm && (
              <Badge variant="outline" className="bg-gray-800 border-green-600 text-green-300 truncate max-w-xs">
                <Search className="mr-1 h-3 w-3" /> "{searchTerm}"
              </Badge>
            )}
          </div>
        ...
        <div className="h-[calc(70vh-150px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
          {selectedRepo ? (
            filteredIssues.length > 0 ? (
              <div className="space-y-4">
                {filteredIssues.map((issue: IssuesData) => (
  ...
  return (
    <div className="container mx-auto mt-4 p-6">
      ...
      <div className="mb-6 bg-gradient-to-r from-gray-900 to-gray-800 p-6 rounded-lg shadow-lg">
        <h1 className="mb-2 font-bold text-4xl text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600 sm:text-6xl">
          Repositories & Issues
        </h1>
